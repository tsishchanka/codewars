...
function findOdd(A) {
    const findRepeats = A.reduce((acc, n) => (acc[n] = (acc[n] || 0) + 1, acc), {})
...
function oddOrEven(array) {
    array.push(0);
    const sum = (array) => array.reduce((prev, curr) => {
        return prev + curr;
    })
...
function positiveSum(arr) {
    if (arr.length == 0) {
        return 0;
    } else return arr.reduce((res, curr) => {
        if (curr >= 0) {
            return res + curr;
        }
        return res;
...
function connotation(str) {
    const negNZ = ['N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z'];
    strArr = str.toUpperCase().replace(/\s+/g, ' ').trim().split(' ');
    const doubleArr = strArr.reduce((res, item) => {
